...
| 31 | P1 | **A8 disk-full has never executed, and its setup was broken** | The one standing unexpected-skip in every campaign: `Drill-A8-DiskFull` (`dev-tests/qa-suite/scripts/07_abuse.ps1:722`) needs admin for `diskpart attach vdisk` and there is no non-elevated equivalent on Windows (`New-VHD`, `fsutil quota` both require it). So disk exhaustion has never been tested on a product whose failure mode under a full disk is data loss. **Reviewed by inspection 2026-09-11 and found broken:** it built a 150MB fixture and copied it onto the 100MB volume, so `Copy-Item` ran out of space before `mediagit add` was ever invoked — the drill would have failed in its own setup and never reached the ENOSPC path it exists to test. Fixed to 55MB with the sizing invariant documented in place (`fixtureMB < usable AND 2*fixtureMB > usable`, since the fixture is incompressible so the ODB copy costs the same again). **Still unrun.** The fix is reasoned, not executed, because verifying it needs the elevation the drill needs. To clear: `Start-Process powershell -Verb RunAs`, then `run_all.ps1` — everything else in the suite runs unelevated. | v0.4.0, needs elevation |
