...
| 32 | P3 | **`MEDIAGIT_PACK_UPLOAD_CONCURRENCY` is still capped at 8, and the reason it was capped is gone** | The default was 8 against a limit of 64 because pack upload held a whole 64 MiB pack in RAM per in-flight pack — a ~512 MiB floor, confirmed by measurement in v0.4.0 (539 MB peak working set on the whole-pack path at concurrency 8). X2 plus the `Bytes` fix brought that to **15.7 MB per extra in-flight pack**, one 16 MiB MPU part, so the arithmetic for raising it is now known rather than guessed: concurrency 32 would cost roughly 39.8 + 31 × 15.7 ≈ **525 MB**, and 64 roughly 1.03 GB. `15_packmem` measures this and will catch a regression. **Not raised in v0.4.0 deliberately**: removing a memory cap is not the same as showing more concurrency helps, and T1 measured 8× concurrency buying only 1.67× on a WAN-bound link — on that link a higher default costs memory and buys nothing. The open question is a FAT link, which this host does not have; anyone raising it should measure there, not here, and should treat ~525 MB at 32 as the budget rather than a surprise. | v0.4.0 measured, not acted on |
