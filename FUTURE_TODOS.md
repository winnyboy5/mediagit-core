...
| 29 | P2 | **GCS has no presigned MPU; feasibility confirmed, not implemented** | Only `s3` and `minio` implement `create_presigned_mpu`; `gcs`, `azure`, `b2_spaces` and `local` inherit the trait default that bails, so a GCS pack upload is one all-or-nothing PUT and a transport failure at 63 of 64 MiB re-sends all 64. This is the ga42 failure class. **Feasibility was the open question and it is now settled:** GCS's v1 SDK uses *resumable* upload (a single sequential session, no parallel byte-range upload), so MPU needs the S3-compatible **XML API** instead — and the prerequisite for that, signing arbitrary query parameters (`?uploads`, `?partNumber=N&uploadId=X`, `?uploadId=X`), IS supported: `SignedUrlBuilder::with_query_param` exists in google-cloud-storage 1.16.0 (`src/storage/signed_url.rs:392`), alongside the `with_method`/`with_expiration` already used by `presign_put`. Scope: mirror `s3.rs`'s trio (~90 lines: create 47, complete 31, abort 12) plus XML parsing for the upload id and part ETags. **Not implemented here, and the reason has changed.** The original reason was that it could not be executed even once without GCS credentials. Those are available on the campaign host (`campaign_env.ps1` exports `GCS_PROJECT_ID` and `GOOGLE_APPLICATION_CREDENTIALS` from `dev-tests\dev-server\config.gcs.toml`), so the blocker now is scope, not capability: v0.4.0 is a hardening release whose remaining cost is two 2.5h campaigns at a single SHA, and adding a new cloud upload path first would mean clearing code that did not exist when the release was scoped. It is a spike with a go/no-go, not a line item — build it on a branch, prove a real GCS push takes the MPU path, and merge only then. Fail-safe if it does land: any MPU failure degrades to the existing single PUT (501/404/part-failure are all handled by `upload_object_mpu`). | v0.4.0, needs credentials |
| 33 | P2 | **Single-PUT pack upload holds a whole 64 MiB pack in RAM — and streaming it is NOT the fix** | `pack_builder.rs` reads the entire pack with `tokio::fs::read` before a single PUT. `Bytes` (B5) only makes the retry CLONES cheap, never the first read. **Measured on a real 10.03 GB push (2026-09-14): 1,075.2 MB peak working set on Azure and 1,075.7 MB on GCS — within 0.5 MB of each other — against 365.2 MB for the same payload on S3.** Not an edge case: neither backend implements `create_presigned_mpu`, so EVERY pack they send takes this branch. **Streaming the body off disk was implemented and REVERTED**, because it breaks retry classification: with a streamed body a server can answer before the client finishes writing, so reqwest reports a TRANSPORT error instead of the HTTP response, and transport errors are retried. `tests/pack_put_retry.rs` caught it immediately — 6/6 clean without the change, 3/6 failing with it, `a_permanent_pack_put_error_is_not_retried` among them. On a real 64 MiB pack against a bad credential that is up to 24 re-uploads of a permanently-doomed request. The MPU path is unaffected because it reads each part into memory. **The real fix is item 29**: give GCS presigned MPU and both the memory and the retry-granularity problems go away together, instead of trading one for the other. | measured + attempted 2026-09-14, blocked on item 29 |
