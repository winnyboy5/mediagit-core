...
| `MEDIAGIT_PACK_SCRUB_INTERVAL_SECS` | `0` (OFF) | 0.4.0 | packs/server | experimental | Seconds between background content-scrub ticks; each tick verifies at most ONE provider-attested pack against its manifest (`main.rs`, `scrub_one_attested_pack`). **Off by default.** Set a positive number of seconds to enable. It is opt-in because it costs BUCKET EGRESS that no local measurement can see: scrubbing a 16 GB repository reads ~10 GB back out of the bucket, billed to your account. With it off, an attested pack that nobody reads is never content-checked -- a real but narrow gap, since attestation already proves the bucket holds the bytes we sent and the read path verifies contents on every read. The scrub skips a tick while the data plane is busy or another pack verification holds the verify permit, so it yields to live transfers rather than competing with them. **Costs bucket egress**: scrubbing a 16 GB repository reads ~10 GB back out of the bucket once. Measured at a 20s interval over 217 packs it added no measurable push time (+2.9%, inside the control arm's own spread) and ~0 MB peak RSS -- so the reason it is off is egress, not throughput. **When to enable:** the read path already catches a bad pack the moment anyone reads it, so the scrub buys EARLIER detection, not detection. That matters when the bucket is the only copy and reads are rare -- archival repositories, cold media -- because corruption found while the source still exists locally is re-pushable, and the same corruption found years later at first read is data loss. Also worth a temporary enable after a provider incident or a suspect deploy. Not worth it for actively-read repositories, where ordinary reads verify continuously for free. |
