...
```bash
# Update version to 0.2.4-beta.1 (example)
...
git commit -m "chore: prepare release 0.2.4-beta.1"
...
git tag -a v0.2.4-beta.1 -m "Release version 0.2.4-beta.1"
...
- [ ] Test Docker image:
  ```bash
  docker run --rm ghcr.io/winnyboy5/mediagit-core:0.2.4-beta.1 --version
  ```
...
```bash
# Alpha
git tag -a v0.2.4-beta.1-alpha.1 -m "Release 0.2.4-beta.1-alpha.1"

# Beta
git tag -a v0.2.4-beta.1-beta.1 -m "Release 0.2.4-beta.1-beta.1"

# Release Candidate
git tag -a v0.2.4-beta.1-rc.1 -m "Release 0.2.4-beta.1-rc.1"
```
...
1. Create hotfix branch from the release tag:
   ```bash
   git checkout -b hotfix/0.2.4-beta.1 v0.2.4-beta.1
   ```

2. Make and commit the fix
3. Update version to patch release (0.2.4-beta.1)
...
2. Delete the git tag locally and remotely:
   ```bash
   git tag -d v0.2.4-beta.1
   git push origin :refs/tags/v0.2.4-beta.1
   ```
...
4. Submit to Chocolatey Community: `choco push mediagit.0.2.4-beta.1.nupkg --source https://push.chocolatey.org/`
