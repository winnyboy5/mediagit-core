...
```bash
# Update version to 0.2.1-beta.2 (example)
...
git commit -m "chore: prepare release 0.2.1-beta.2"
...
git tag -a v0.2.1-beta.2 -m "Release version 0.2.1-beta.2"
...
- [ ] Test Docker image:
  ```bash
  docker run --rm ghcr.io/winnyboy5/mediagit-core:0.2.1-beta.2 --version
  ```
...
```bash
# Alpha
git tag -a v0.2.1-beta.2-alpha.1 -m "Release 0.2.1-beta.2-alpha.1"

# Beta
git tag -a v0.2.1-beta.2-beta.1 -m "Release 0.2.1-beta.2-beta.1"

# Release Candidate
git tag -a v0.2.1-beta.2-rc.1 -m "Release 0.2.1-beta.2-rc.1"
```
...
1. Create hotfix branch from the release tag:
   ```bash
   git checkout -b hotfix/0.2.1-beta.2 v0.2.1-beta.2
   ```

2. Make and commit the fix
3. Update version to patch release (0.2.1-beta.2)
...
2. Delete the git tag locally and remotely:
   ```bash
   git tag -d v0.2.1-beta.2
   git push origin :refs/tags/v0.2.1-beta.2
   ```
...
4. Submit to Chocolatey Community: `choco push mediagit.0.2.1-beta.2.nupkg --source https://push.chocolatey.org/`
