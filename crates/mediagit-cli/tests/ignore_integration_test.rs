...
fn test_add_skips_ignored_files() {
    ...
    assert!(!status_out.contains("file.tmp"), "file.tmp should be ignored");
    assert!(!status_out.contains("debug.log"), "debug.log should be ignored");
}
...
fn test_add_force_overrides_ignore() {
    ...
    let (status_out, _) = run(root, &["status", "--porcelain"]);
    assert!(status_out.contains("file.tmp"), "file.tmp should be staged with --force");
}
...
fn test_status_hides_ignored_from_untracked() {
    ...
    assert!(!status_out.contains("secret.tmp"), "ignored file should not be in untracked list");
    // normal.txt should appear as untracked
    assert!(status_out.contains("normal.txt"), "normal file should appear as untracked");
}
...
fn test_status_shows_ignored_with_flag() {
    ...
    assert!(status_out.contains("ignored.tmp"), "ignored file should appear with --ignored flag");
}
...
fn test_status_porcelain_ignored() {
    ...
    assert!(status_out.contains("!! "), "porcelain output should use !! prefix for ignored");
    assert!(status_out.contains("app.log"), "ignored file should be listed");
}
...
fn test_ignore_directory_pattern() {
    ...
    let (status_out, _) = run(root, &["status", "--porcelain"]);
    assert!(status_out.contains("src/main.rs"), "src/main.rs should be staged");
    assert!(!status_out.contains("build/output.bin"), "build/ dir should be ignored");
    assert!(!status_out.contains("build/artifact.o"), "build/ dir should be ignored");
}
...
fn test_ignore_negation() {
    ...
    assert!(status_out.contains("important.log"), "important.log should be staged (negation)");
    assert!(!status_out.contains("debug.log"), "debug.log should be ignored");
}
...
fn test_no_ignore_file_behavior_unchanged() {
    ...
    let (status_out, _) = run(root, &["status", "--porcelain"]);
    assert!(status_out.contains("file.txt"), "file.txt should be staged");
    assert!(status_out.contains("file.tmp"), "file.tmp should be staged (no ignore file)");
}
