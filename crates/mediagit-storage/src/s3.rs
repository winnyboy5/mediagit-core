...
impl StorageBackend for S3Backend {
    ...
    > {
        ...
        // `failed` fuses the stream: after an error we yield it once and stop.
        // Continuing to poll the same reader could resume mid-object and hand a
        // consumer a gap-spliced byte sequence that looks like valid data.
    ...
    async fn create_presigned_mpu(
        ...
        Ok(Some(crate::PresignedMpu {
            ...
            parts,
            part_size,
            // UNATTESTED. This backend is reached only by `b2_spaces` — the
            // server builds its "aws" backend from `MinIOConfig`
            // (handlers/mod.rs), so attestation lives in `minio.rs`. An earlier
            // implementation here was a no-op for every push and was reverted;
            // attesting B2/Spaces means checking what those providers actually
            // validate, not copying the S3 shape on the assumption it carries
            // over.
            checksum: None,
        }))
    }
