...
impl ProtocolClient {
    ...
    pub fn new(base_url: impl Into<String>) -> Self {
        // Control-plane pool size: how many idle TLS connections to keep warm
        // per host (mediagit-server). High values help URL-mint bursts that fire
        // many small requests in rapid succession. Override via
        // MEDIAGIT_HTTP_POOL_MAX; default 64 matches DOWNLOAD_CONCURRENCY.
        let pool_max: usize = std::env::var("MEDIAGIT_HTTP_POOL_MAX")
            .ok()
        ...
        for oid in chunked_oids.iter() {
            ...
            if missing_chunks.is_empty() {
                ...
                if !full_chunks.is_empty() {
                    ...
                    // window). Pool sized to upload concurrency so every worker
                    // gets a warm socket without stacking up idle sockets.
                    let data_pool_max: usize = std::env::var("MEDIAGIT_HTTP_POOL_MAX")
                        .ok()
                        .and_then(|s| s.parse().ok())
                        .filter(|n: &usize| *n > 0)
                        .unwrap_or(96);
                    let direct_client = reqwest::Client::builder()
                        ...
                        .pool_max_idle_per_host(data_pool_max)
        ...
        F: FnMut(u64, u64, &str),
    {
        ...
        // Pool sized to concurrent_downloads × range_parallel (24 × 4 = 96) so
        // every in-flight range-GET reuses a warm TCP session.
        let dl_pool_max: usize = std::env::var("MEDIAGIT_HTTP_POOL_MAX")
            .ok()
            .and_then(|s| s.parse().ok())
            .filter(|n: &usize| *n > 0)
            .unwrap_or(160);
        let direct_client = reqwest::Client::builder()
            .pool_idle_timeout(std::time::Duration::from_secs(60))
            .pool_max_idle_per_host(dl_pool_max)
            .tcp_keepalive(std::time::Duration::from_secs(45))
            .tcp_nodelay(true)
            .http1_only()
            .build()
        ...
        for oid in chunked_oids.iter() {
            ...
            total_manifest_bytes += manifest.chunks.iter().map(|c| c.size as u64).sum::<u64>();
            ...
            if !missing_chunks.is_empty() {
                ...
                if !full_chunks.is_empty() {
                        ...
                        .map(|chunk_id| {
                            ...
                            async move {
                                ...
                                if let Some(Some(presigned)) = download_urls.get(&hex) {
                                    ...
                                    {
                                        Ok(data) => {
                                            return Ok::<_, anyhow::Error>((chunk_id, data))
                                        }
                                        Err(e) => {
                                            tracing::debug!(
                                                chunk = %hex,
                    ...
                    while let Some(result) = stream.next().await {
                        ...
                        _dl_pass_a_n += 1;
                        _dl_pass_a_bytes += chunk_data.len() as u64;
...
async fn download_chunk_direct(
    ...
    // Note: chunk ID = SHA-256(uncompressed data), but S3 stores compressed bytes.
    // We cannot verify the hash here without decompressing; TLS + S3 ETag guarantee
...
/// Verifies SHA-256 over the reassembled body.
