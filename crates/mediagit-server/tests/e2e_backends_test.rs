...
fn generate_test_jpeg() -> Vec<u8> {
    ...
    vec![
        ...
        0xDB, 0x2C, 0xA2, 0x8A, 0x28, 0x03, 0xFF, 0xD9,
    ]
}
...
fn generate_test_png() -> Vec<u8> {
    ...
    vec![
        ...
        0x08, 0xD7, 0x63, 0xF8, 0xCF, 0xC0, 0x00, 0x00, 0x00, 0x03, 0x00, 0x01, 0x00, 0x18, 0xDD,
...
async fn test_local_backend_complete_flow() {
    ...
    let main_ref_info = refs_response
        .refs
        .iter()
...
async fn test_minio_backend_complete_flow() {
    ...
    // Cleanup
    storage_arc
        .delete(&format!("objects/{}", oid.to_hex()))
        .await
        .unwrap();
...
async fn test_minio_concurrent_uploads() {
    ...
    // Cleanup
    storage_arc
        .delete(&format!("objects/{}", oids[0].to_hex()))
        .await
        .unwrap();
}
